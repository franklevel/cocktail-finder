...
import { fetchDrinksActionThunk, resetDrinksActionThunk, setSearchQueryActionThunk } from "../redux/actions/drinksActions";
...
const DetailScreen = ({ navigation }: any) => {
  ...
  const onChange = (e: NativeSyntheticEvent<TextInputChangeEventData>): void => {
    ...
    if (drink && drink.length > 2) {
      dispatch(fetchDrinksActionThunk(drink));
    }
  }
    ...
    <>
      <View style={styles.topBarContainer}>
        ...
        <TouchableOpacity onPress={() => {
          dispatch(setSearchQueryActionThunk(""))
          dispatch(resetDrinksActionThunk())
        }} ><Text style={styles.topBarButton}>{CANCEL_BUTTON_TEXT}</Text></TouchableOpacity>
      </View>
